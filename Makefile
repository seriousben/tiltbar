...
# Generate BuildInfo.swift at build time.
# Version is read from .release-please-manifest.json (the sole source of truth).
# Also determines dev-build status from git state (dirty tree or not on release tag).
...
generate-build-info:
	...
	echo "let appVersion = \"$$VERSION\"" >> Sources/TiltBar/BuildInfo.swift; \
